...
\title[Journal club crítico]{Journal club crítico}
...
\begin{frame}{La pregunta que guía el trabajo}
  \transdissolve[duration=0.25]
  {\large ¿La evidencia del artículo sostiene su afirmación principal?}
  \medskip
...
\begin{frame}{Una estrategia explícita}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[x=1cm,y=1cm,font=\footnotesize]
      ...
      \node[text=sat1843819360, align=center, text width=1.75cm, inner sep=1pt] at (1.113,-1.254) {\bfseries Citar el artículo real};
      \node[text=sat1843819360, align=center, text width=1.75cm, inner sep=1pt] at (3.976,-1.254) {\bfseries Reconstruir diseño y controles};
      \node[text=sat1843819360, align=center, text width=1.75cm, inner sep=1pt] at (6.620,-1.254) {\bfseries Examinar figuras y estadística};
      \node[text=sat1226267613, align=center, text width=1.75cm, inner sep=1pt] at (9.264,-1.254) {\bfseries Discutir validez externa};
    \end{tikzpicture}
  \end{figure}
\end{frame}
\note{%
...
\begin{frame}{Mostrar la evidencia}
  ...
  \begin{table}
    \centering
    \caption{Guía de lectura: sustituir por un artículo verificable.}
    \begin{tabular}{cc}
      \toprule
      \textbf{Elemento} & \textbf{Qué extraer del artículo} \\
      \midrule
      Pregunta & Población o sistema y comparación \\
      Evidencia & Mediciones y controles \\
      Limitación & Sesgos y alternativas \\
      Reproducción & Datos y código disponibles \\
      \bottomrule
    \end{tabular}
  \end{table}
\end{frame}
...
\begin{frame}{Interpretación y límites}
  \transdissolve[duration=0.25]
  \begin{itemize}
    \item No se incluye una referencia inventada.
    \item Importa un DOI real desde Referencias o conecta Zotero voluntariamente.
    \item Distingue la afirmación del autor de tu evaluación crítica.
  \end{itemize}
\end{frame}
\note{%
  {\scriptsize\textbf{Tiempo previsto: 1:30 min}}\par\medskip
...
\begin{frame}{Antes de compartir}
  \transdissolve[duration=0.25]
  \begin{itemize}
    \item DOI, versión y posibles correcciones
    \item Tamaño muestral y unidad experimental
    \item Disponibilidad de datos y reproducibilidad
  \end{itemize}
  \medskip
